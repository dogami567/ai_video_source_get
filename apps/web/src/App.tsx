  ...
  const onChatSend = async () => {
    ...
    if (!consent?.consented) {
      ...
      if (url) {
        ...
        setConsentModalAutoConfirm(consent?.auto_confirm ?? true);
        setConsentModalError(null);
        setConsentModalNextAction({ kind: "chatSend", chatId: chatIdToUse, message, attachments: attachmentsPayload });
        setConsentModalOpen(true);
        return;
      }
    }
    ...
    setChatSendBusy(true);
    try {
      const resp = await runChatTurn({ chatId: chatIdToUse, message, attachments: attachmentsPayload });
      setChatDraft("");
      setChatAttachments([]);
